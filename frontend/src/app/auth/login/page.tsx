...
export default function Login() {
  ...
  const handleSendOTP=()=>{
    setError('')
    if(!email){
      setError('ادخل الايميل')
      return
    }

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
    if(!emailRegex.test(email)){
      setError('صيغة الايميل غير صحيحة')
      return
  ...
  return (
      ...
      <p className="text-gray-600 text-sm">أدخل الايميل الخاص بك لتلقي رمز التحقق </p>
      <div className="flex flex-col items-end text-right">
        ...
        <button
          ...
          className={`w-86 flex items-center justify-center bg-[#3B82F6] text-white font-medium py-2 rounded-lg transition-all duration-200 ${
            loading ? 'cursor-not-allowed bg-[#2563EB]/50' : 'hover:bg-[#2563EB]'
          }`}
        >
          {loading && (
            <div
